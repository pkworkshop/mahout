...
public class LinearCrossValidation extends AbstractJob {
  private static final String DIRECTORY_CONTAINING_CONVERTED_INPUT = "data";
  private static final int formatWidth = 8;

  private class LinearCrossValidationParameter {
    public int numOfCV;
  ...
  private LinearCrossValidationParameter parameter;
  ...
  private LinearCrossValidation() {
  }

  private static final Logger log = LoggerFactory.getLogger(LinearCrossValidation.class);

  public static void main(String[] args) throws Exception {
    ToolRunner.run(new Configuration(), new LinearCrossValidation(), args);
  }

  private boolean validateParameter(LinearCrossValidationParameter parameter, String featureNames, String separator) {
  ...
  private void printInfo(LinearCrossValidationParameter parameter, PenalizedLinearSolver solver)
      throws IOException {
    ...
    if (parameter.intercept) {
      model += " " + String.format("%" + Integer.toString(formatWidth) + ".5f", coefficients.beta0) + " * intercept";
    } else {
      model += " " + "0";
    }
    int index = 0;
    if (!parameter.independent.equals("")) {
      String[] independent = parameter.independent.split(",");
      for (int i = 0; i < independent.length; ++i) {
        model += " + " + String.format("%" + Integer.toString(formatWidth) + ".5f", coefficients.beta[index++]) + " * " + independent[i];
      }
    }
    if (!parameter.interaction.equals("")) {
      String[] interaction = parameter.interaction.split(",");
      for (int i = 0; i < interaction.length; ++i) {
        model += " + " + String.format("%" + Integer.toString(formatWidth) + ".5f", coefficients.beta[index++]) + " * " + interaction[i];
      }
    }
    System.out.println();
    System.out.println(model);
    System.out.println("Optimal lambda is: " + String.format("%" + Integer.toString(formatWidth) + ".5f", solver.getOptLambda()[0]));
    System.out.println("The training and (test) errors are in file: " + output + "/trainingTestingError.txt");
    double[] trainError = solver.getTrainError();
    double[] testError = solver.getTestError();
    FileSystem fs = FileSystem.get(getConf());
    BufferedWriter br = new BufferedWriter(new OutputStreamWriter(fs.create(new Path(output, "trainingTestingError.txt"), true)));
    for (int i = 0; i < lambdas.length; ++i) {
      String line = "" + lambdas[i] + " " + trainError[i] + " " + testError[i];
      br.write(line + "\n");
    }
    br.close();
  }
  ...
  private boolean parseArgs(String[] args) throws IOException {
    ...
    parameter.numOfCV = Integer.parseInt(getOption("numOfCV"));
  ...
  private boolean processLambda(LinearCrossValidationParameter parameter) {
    String lambdaSeq = "";
